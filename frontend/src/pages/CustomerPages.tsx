...
export function Dashboard() {
  const {user} = useAuth();
  return <><PageTitle eyebrow="Your account" title="Welcome to the table"/><p className="intro">{user?.email}</p><div className="card-grid">{user?.role === 'CUSTOMER' && <><section className="panel"><h2>Your next gathering</h2><p>Explore the menus and send us your event details.</p><Link className="button" to="/packages">Browse packages</Link></section><section className="panel"><h2>Keep track</h2><p>See your booking references, details and review status.</p><Link className="button secondary" to="/bookings">My bookings</Link></section><section className="panel"><h2>A little about you</h2><p>Keep your contact information up to date.</p><Link className="button secondary" to="/profile">Manage profile</Link></section></>}{user && catalogRoles.includes(user.role) && <section className="panel"><h2>Menus & packages</h2><p>Maintain the dishes, prices and packages customers can choose.</p><Link className="button" to="/staff/catalog">Manage catalog</Link></section>}{user && reviewRoles.includes(user.role) && <section className="panel"><h2>Booking requests</h2><p>Review event details and approve or reject requests.</p><Link className="button" to="/staff/bookings">Review bookings</Link></section>}{user?.role === 'ADMIN' && <section className="panel"><h2>User access</h2><p>Maintain staff roles and account availability.</p><Link className="button" to="/admin/users">Manage users</Link></section>}</div>{user && ['OPERATIONS_MANAGER','ACCOUNTS_EXECUTIVE'].includes(user.role) && <p className="empty">Your account is active. Tools for your role will be available in a later development phase.</p>}</>;
}
